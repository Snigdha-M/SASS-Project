/* Adapted from "SASS Project.sas" (Chicago Energy Benchmarking analysis).
   ...
   the bundle is self-contained. Analysis logic (block 8: building-age
   bucketing + clustered EUI chart, block 9: chi-square association
   between age group and energy rating) is unmodified from the source
   script, other than replacing YEAR(TODAY()) with a fixed reference
   year so the bucketing is reproducible on any run date. */
...
/* Reference year fixed (was year(today())) so bucketing is reproducible across runs */
%let ref_year = 2026;

proc sql;
  create table energy_data_age_groups as
  select Year_Built,
         (&ref_year - Year_Built) as Building_Age,
         case
           when (&ref_year - Year_Built) <= 10 then '0-10 years'
           when (&ref_year - Year_Built) <= 20 then '11-20 years'
           when (&ref_year - Year_Built) <= 30 then '21-30 years'
           when (&ref_year - Year_Built) <= 40 then '31-40 years'
           else 'Over 40 years'
         end as Age_Group,
...
/* 9 Categorical Association test between age_group and Energy rating - Astha */
